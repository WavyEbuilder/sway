...
#if HAVE_SELINUX
// TODO: This is quite messy.  Maybe fetch and trim the names of the globals
//       with ->name?  Might make it a bit harder to have the permission for
//       e.g. wlr and ext protocols be the same (which is more than just a
//       usability thing, keeping them the same means backwards compatibility).
